...
void luRu() {
	...
	while (1) {
		...
		while (1) {
			...
			if (inputNum >= 10000000000 && inputNum <= 99999999999) {
				...
				if (isStuNumExists(head, inputNum)) {
					printf("Error:学号已存在，请重新输入。\n");
					continue;
				}
				printf("学号未重复！\n");
				printf("Finish:学号合法！已录入。\n");
		...
		while (1) {
			...
			if (strcmp(newNode->sex, "男") == 0
			        || strcmp(newNode->sex, "女") == 0) {  // 对比判断链表中的值进行检测;讲解：strcmp与strstr的区别
				printf("Finish:性别规则合法！（规则1）已录入。\n");
				break;
			} else {
				if (strcmp(newNode->sex, "其他") == 0) {
					printf("请输入其他性别：");
					scanf("%99s", newNode->sex);
					printf("Finish:性别规则合法！（规则2）已录入。\n");
					break;
					// 规则1：限“男”“女”。 规则2：任意性别
				} else {
					printf("Error:无效性别！请重新输入。\n");
		...
		while (1) {
			...
			if (inputNum >= 10000000000 && inputNum <= 19999999999) { // 必须1为开头的11位手机号
				...
				printf("Finish:号码字数合法！已录入\n");
				break;
			} else {
				printf("Error:非法号码！请检查字数并重新输入。\n");
				clearInputBuffer();                     // 清除缓存防止死循环
				continue;
			}
		}                                               // 录入手机号码
		while (1) {
			...
			if (strstr(inputEmail, "@gmail.com") != NULL) {
				printf("Finish:格式正确（gmail邮箱），已录入。\n");
//                newNode->emailNum = inputEmail;        // 错误
				strcpy(newNode->emailNum,
				       inputEmail);   // 对于字符数组，不能通过简单的赋值来进行复制，应该使用字符串操作函数如 strcpy。
				break;
			}
			if (strstr(inputEmail, "@qq.com") != NULL) {
				printf("Finish:格式正确（QQ邮箱），已录入。\n");
				strcpy(newNode->emailNum, inputEmail);
				break;
			}
			if (strstr(inputEmail, "@163.com") != NULL) {
				printf("Finish:格式正确（网易邮箱），已录入。\n");
				strcpy(newNode->emailNum, inputEmail);
				break;
			}
			if (strstr(inputEmail, "@outlook.com") != NULL) {
				printf("Finish:格式正确（Outlook邮箱），已录入。\n");
				strcpy(newNode->emailNum, inputEmail);
				break;
			}
			if (strstr(inputEmail, "@yahoo.com") != NULL) {
				printf("Finish:格式正确（雅虎邮箱1），已录入。\n");
				strcpy(newNode->emailNum, inputEmail);
				break;
			}
			if (strstr(inputEmail, "@myyahoo.com") != NULL) {
				printf("Finish:格式正确（雅虎邮箱2），已录入。\n");
				strcpy(newNode->emailNum, inputEmail);
				break;
			} else {
				printf("Error:格式错误!可能是未受支持的邮箱格式，请重新输入\n");
				continue;
			}
		}                                               // 录入邮箱
...
void xiuGai() {
	...
	// 查找满足条件的记录
	Contact_Person *temp =
	    head;             // 1.定义指针变量temp类型为Contact_Person自定义结构体，类型为，2.将temp赋值为head，使temp指向链表的第一个节点。
	while (temp != NULL) {
...
//    }
	...
	fclose(file);
	printf("Finish:数据已保存到文件：“data.txt\n");
	system("pause");
}                                   // 保存数据（链表）
void readData() {
    ...
    char line[1000000];
    struct contactPerson *head = NULL;
    while (fgets(line, sizeof(line), file)) {
        long long data = atoll(line);
        struct contactPerson *newNode = (struct contactPerson *) malloc(sizeof(struct contactPerson));
        newNode->stuNum = data;
        newNode->pNext = NULL;
        if (head == NULL) {
            head = newNode;
        } else {
            struct contactPerson *temp = head;
...
//    printf("打开成功\n");
//    Contact_Person *head = NULL;
//    Contact_Person *new_p = NULL;
//    Contact_Person *p = NULL;
//    while (1) {
//        new_p = (Contact_Person *) malloc(sizeof(Contact_Person));
//        if (new_p == NULL) {
//            printf("内存分配失败\n");
//            return;
//        }
//        if (fscanf(file, "%lld  %s    %s    %s    %s    %s    %s    %lld    %s\n", &new_p->stuNum, new_p->name, new_p->sex, new_p->academic,
//                   new_p->major, new_p->province,new_p->address, &new_p->phoneNum, new_p->emailNum) != 9) {
//            printf("读取到的数不等于9\n");
//            free(new_p);
//            break;
//        } // 是从文件中读取数据并将其存储到链表节点 new_p 中
//        new_p->pNext = NULL;
//        if (head == NULL) {
//            head = new_p;
//            p = head;
...
luRu_Start:
				...
				while (1) {
					...
					printf("是否继续录入成绩（T/F）：");
					scanf("%9s", TFChoice);
					if (strcmp(TFChoice, "F") ==
					        0) {                 // strcmp 是一个用于比较两个字符串的函数。返回值为 0 表示两个字符串相等，否则返回非零值，表示两个字符串不相等。
						printf("录入结束\n");
						break;
					} else {
